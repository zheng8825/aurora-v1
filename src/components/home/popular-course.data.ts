import type { Course } from '@/interfaces/course'

export const data: Array<Course> = [
  {
    id: 1,
    cover: '/images/courses/a9e7b27a0c5e986a22416d79e2e9dba9.jpg',
    title: 'Solar photovoltaic (PV) System',
    rating: 5,
    ratingCount: 8,
    price: 25,
    category: 'Beginner',
    description: 'Harnessing sunlight and converting into sustainable electricity.',
  },
  {
    id: 2,
    cover: '/images/courses/alvaro-reyes-qWwpHwip31M-unsplash.jpg',
    title: 'Micro Wind Turbine',
    rating: 5,
    ratingCount: 15,
    price: 20,
    category: 'Intermediate',
    description: 'Harnesses even gentle breezes for electricity.',
  },
  {
    id: 3,
    cover: '/images/courses/christopher-gower-m_HRfLhgABo-unsplash.jpg',
    title: 'Energy Storage Solution',
    rating: 4,
    ratingCount: 7,
    price: 30,
    category: 'Beginner',
    description:
      'Energy Storage Solution captures excess energy from Renewable Energy sources, storing it for later use or when demand exceeds generation.',
  },
  // {
  //   id: 4,
  //   cover: '/images/courses/true-agency-o4UhdLv5jbQ-unsplash.jpg',
  //   title: 'The Complete Guide Docker and Kubernetes',
  //   rating: 4,
  //   ratingCount: 12,
  //   price: 30,
  //   category: 'Intermediate',
  // },
  // {
  //   id: 5,
  //   cover: '/images/courses/stillness-inmotion-Jh6aQX-25Uo-unsplash.jpg',
  //   title: 'Modern React with MUI & Redux',
  //   rating: 4,
  //   ratingCount: 32,
  //   price: 35,
  //   category: 'Intermediate',
  // },
  // {
  //   id: 6,
  //   cover: '/images/courses/stillness-inmotion-YSCCnRGrD-4-unsplash.jpg',
  //   title: 'Ethical Hacking Bootcamp Zero to Mastery',
  //   rating: 5,
  //   ratingCount: 14,
  //   price: 35,
  //   category: 'Beginner',
  // },
  // {
  //   id: 7,
  //   cover: '/images/courses/grovemade-RvPDe41lYBA-unsplash.jpg',
  //   title: 'Adobe Lightroom For Beginners: Complete Photo Editing',
  //   rating: 4,
  //   ratingCount: 6,
  //   price: 25,
  //   category: 'Beginner',
  // },
]
